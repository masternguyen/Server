...
module.exports = function (client, data) {
  if (!!data && !!data.cuoc && Array.isArray(data.line)) {
    ...
    if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
      client.red({
        Zeus: { status: 0 },
        notice: { text: "ข้อมูลไม่ถูกต้อง...", title: "ความล้มเหลว" },
      })
    } else {
      client.Zeus =
        void 0 === client.Zeus
      ...
      UserInfo.findOne({ id: client.UID }, "red name", function (err, user) {
        if (!user || (client.Zeus.free === 0 && user.red < tongCuoc)) {
          client.red({
            Zeus: { status: 0, notice: "คุณมี R ไม่เพียงพอที่จะหมุน.!!" },
          })
        } else {
          ...
          HU.findOne({ game: "Zeus", type: bet }, {}, function (err2, dataHu) {
            ...
            let celSS = null
            if (line.length < 15) {
              console.log("Zeus, จำนวนบรรทัดที่เลือก: " + line.length)
              celSS = [
                random_T3(),
                2,
                random_T3(),
                ...
                function (err4, small) {
                  client.red({
                    Zeus: {
                      ...
                      line_win: result,
                      win: bet_win,
                      free: client.Zeus.free,
                      isFree: isFree,
                      isBonus: client.Zeus.bonusL,
                      isNoHu: nohu,
                      isBigWin: isBigWin,
                      phien: small.id,
                    },
