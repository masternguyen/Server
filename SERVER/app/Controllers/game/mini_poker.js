...
function spin(client, data) {
  if (!!data && !!data.cuoc) {
    ...
    if (!(bet == 100 || bet == 1000 || bet == 10000)) {
      client.red({
        mini: { poker: { status: 0 } },
        notice: { text: "ข้อมูลไม่ถูกต้อง...", title: "MINI POKER" },
      })
    } else {
      UserInfo.findOne({ id: client.UID }, "red name", function (err, user) {
        if (!user || user.red < bet) {
          client.red({
            mini: { poker: { status: 0, notice: "ไม่พอ RED เพื่อยิง.!!" } },
          })
        } else {
          let UID = client.UID
          ...
          let dongChat = ketqua_temp.filter(
            (type_card) => type_card.type === typeCard
          ) // Kiểm tra đồng chất
          dongChat = dongChat.length == 5 ? true : false // Dây là đồng chất

          let AK = ketqua_temp.sort(function (a, b) {
            return a.card - b.card
          }) // sắp sếp từ A đến K (A23...JQK)
          let isDay = false // là 1 dây
          if (bo3 == false && bo2 == 0 && tuQuy == null) {
            if (
              AK[4].card - AK[0].card === 4 ||
              (AK[4].card - AK[1].card === 3 &&
                AK[0].card === 0 &&
                AK[4].card === 12)
            ) {
              isDay = true
          ...
          HU.findOne(
            { game: "minipoker", type: bet },
            "name bet min toX balans x",
            function (err, dataHu) {
              if (dataHu === null) {
                return void 0
              }
              ...
              if (checkName || (dongChat && isDay && AK[4].card > 9)) {
                ...
                if (checkName) {
                  ...
                  card = base_card.card.filter(function (cardT) {
                    if (
                      randomMin == 9 &&
                      cardT.card == 0 &&
                      cardT.type == randomType
                    ) {
                      return true
                    } else if (
                      cardT.card >= randomMin &&
                      cardT.card < randomMin + 5 &&
                      cardT.type == randomType
                    ) {
                      return true
                    }
                ...
                client.redT.sendInHome({
                  pushnohu: { title: "MINI POKER", name: name, bet: an },
                })
                ...
                client.redT.sendInHome({
                  news: {
                    t: { game: "MINI POKER", users: name, bet: an, status: 2 },
                  },
                })
              } else if (tuQuy != null) {
                // x150     TỨ QUÝ (TỨ QUÝ)
                ...
                // x2.5	 	1 ĐÔI > J
                an = bet * 2.5
                text = "คู่ " + base_card.name[bo2_a[0]]
              }
              card = null
              let tien = an - bet
              ...
              if (tien < 0) {
                huUpdate["redLost"] =
                  uInfo["redLost"] =
                  mini_users["lost"] =
                    tien * -1 // Cập nhật Số Red đã Thua
              }
              if (code === 2) {
                  ...
                  if (err) {
                    client.red({
                      mini: {
                        poker: {
                          status: 0,
                          notice: "เกิดข้อผิดพลาด โปรดลองอีกครั้ง.!!",
                        },
                      },
                    })
                  } else {
              ...
              miniPokerUsers
                .updateOne(
                  { uid: UID },
                  {
                    $set: { time: new Date().getTime(), select: bet },
                    $inc: mini_users,
                  }
                )
                .exec()
              ...
              if (!!vipStatus && vipStatus.status === true) {
                TopVip.updateOne({ name: name }, { $inc: { vip: bet } }).exec(
                  function (errV, userV) {
                    if (!!userV && userV.n === 0) {
                      try {
...
function log(client, data) {
  if (!!data && !!data.page) {
    ...
    if (page < 1) {
      ...
      let kmess = 9
      miniPokerRed
        .countDocuments({ name: client.profile.name })
