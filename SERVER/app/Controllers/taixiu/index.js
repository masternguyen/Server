  ...
  if (!!str) {
    UserInfo.findOne({ id: client.UID }, "red", function (err, user) {
      if (!user || user.red < 1000) {
        client.red({
          taixiu: { err: "บัญชีต้องมีอย่างน้อย R 1,000 เพื่อแชท.!!" },
        })
        client = null
        str = null
      } else {
        if (!validator.isLength(str, { min: 1, max: 250 })) {
          client.red({ taixiu: { err: "จำนวนอักขระตั้งแต่ 1 - 250." } })
          client = null
          str = null
        } else {
                  ...
                  function (isCuocErr, isCuoc) {
                    if (!!isCuoc) {
                      ...
                      if (isCuoc.select !== select) {
                        client.red({ taixiu: { err: "เดิมพันข้างเดียว.!!" } })
                      } else {
  ...
  if (!!data && !!data.phien) {
    ...
    Promise.all([getPhien, getCuoc]).then((values) => {
      if (!!values[0]) {
        ...
        client.red({
          notice: { title: "ข้อผิดพลาด", text: "ไม่มีเซสชัน...", load: false },
        })
        phien = null
        getPhien = null
        //getCuoc  = null;
