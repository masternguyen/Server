  ...
  HU.findOne({ game: "vuongquocred", type: bet }, {}, function (err2, dataHu) {
    ...
    Promise.all(
      ...
      result.forEach(function (line_win) {
        if (line_win.win == 5) {
          if (line_win.type === 5) {
            ...
            io.sendInHome({
              pushnohu: { title: "Ngộ Không", name: user.name, bet: okHu },
            })
            nohu = true
            VuongQuocRed_red.create({
              name: user.name,
              type: 2,
              win: bet_win,
              bet: bet,
      ...
      if (!nohu && bet_win >= tongCuoc * 2.24) {
        io.sendInHome({
          news: {
            t: { game: "หวู่คง", users: user.name, bet: bet_win, status: 2 },
          },
        })
      }
      HU.updateOne(
      ...
      if (!!vipConfig && vipConfig.status === true) {
        TopVip.updateOne({ name: user.name }, { $inc: { vip: tongCuoc } }).exec(
          function (errV, userV) {
            if (!!userV && userV.n === 0) {
              try {
