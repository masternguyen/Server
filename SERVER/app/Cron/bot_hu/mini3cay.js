...
let spin = function (io, user) {
  ...
  // Sử lý bài
  let an = 0
  let card = [...base_card.card].slice(0, 36)
  ...
  let LienTiep = ketqua.sort(function (a, b) {
    return a.card - b.card
  })
  let Day =
    LienTiep[2].card - LienTiep[0].card === 2 &&
    LienTiep[2].card != LienTiep[1].card &&
    LienTiep[1].card != LienTiep[0].card
      ? true
      : false // Bộ liên tiếp
  ...
  HU.findOne({ game: "mini3cay", type: cuoc }, {}, function (err, data) {
    ...
    if (bo3 && bo3_a === 0) {
      // NỔ HŨ
      HU.updateOne(
        { game: "mini3cay", type: cuoc },
        { $set: { name: "", bet: data.min } }
      ).exec()
      an = Math.floor(data.bet - Math.ceil((data.bet * 2) / 100))
      io.sendInHome({
        pushnohu: { title: "มินิ 3 พืช", name: user.name, bet: an },
      })
    } else if (Day && dongChat) {
      an = cuoc * 30
      io.sendInHome({
        news: {
          t: { game: "มินิ 3 พืช", users: user.name, bet: an, status: 2 },
        },
      })
    } else if (bo3) {
      an = cuoc * 20
