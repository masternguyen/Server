...
let thongtin_thanhtoan = function thongtin_thanhtoan(dice = null) {
  if (!!dice) {
      ...
      if (list.length) {
        Promise.all(
          list.map(function (cuoc) {
            ...
            let tongDat =
              cuoc[0] + cuoc[1] + cuoc[2] + cuoc[3] + cuoc[4] + cuoc[5]
            let update = {}
            let updateGame = {}
            cuoc.thanhtoan = true
            ...
            if (void 0 !== io.users[cuoc.uid]) {
              ...
              if (TongThang > 0) {
                status = {
                  mini: { baucua: { status: { win: true, bet: TongThang } } },
                }
              } else {
                status = {
                  mini: {
                    baucua: { status: { win: false, bet: Math.abs(totall) } },
                  },
                }
              }
          ...
          if (arrayOfResults.length) {
            ...
            Promise.all(
              arrayOfResults.map(function (obj) {
                return {
                  users: obj.users,
                  bet: obj.bet,
                  game: "ปูม้าเลือกตั้ง",
    ...
    Object.values(io.users).forEach(function (users) {
      users.forEach(function (client) {
        if (
          ...
          client.gameEvent.viewBauCua !== void 0 &&
          client.gameEvent.viewBauCua
        ) {
    ...
    let admin_data = {
      baucua: { info: io.baucua.infoAdmin, ingame: io.baucua.ingame },
    }
